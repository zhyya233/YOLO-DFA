...
class BaseTrainer:
    ...
    def build_optimizer(model, name='Adam', lr=0.001, momentum=0.9, decay=1e-5):
        ...
        """
        g0, g1, g2 = [], [], []  # g0: weights (with decay), g1: bn weights (no decay), g2: biases (no decay)
        bn = tuple(v for k, v in nn.__dict__.items() if 'Norm' in k)  # normalization layers, i.e. BatchNorm2d()
        processed_ids = set()  # To avoid duplicate parameter assignments
        ...
        for m in model.modules():
            for p_name, p in m.named_parameters(recurse=False):
                ...
                if p_name == 'bias':
                    g2.append(p)  # bias (no decay)
                elif isinstance(m, bn):
                    g1.append(p)  # bn weight (no decay)
                else:
                    g0.append(p)  # other weights (with decay)

        if name == 'Adam':
            optimizer = torch.optim.Adam(g2, lr=lr, betas=(momentum, 0.999))  # adjust beta1 to momentum
        elif name == 'AdamW':
            optimizer = torch.optim.AdamW(g2, lr=lr, betas=(momentum, 0.999), weight_decay=0.0)
        elif name == 'RMSProp':
            optimizer = torch.optim.RMSprop(g2, lr=lr, momentum=momentum)
        elif name == 'SGD':
            optimizer = torch.optim.SGD(g2, lr=lr, momentum=momentum, nesterov=True)
        else:
            raise NotImplementedError(f'Optimizer {name} not implemented.')

        optimizer.add_param_group({'params': g0, 'weight_decay': decay})  # add g0 with weight_decay
        optimizer.add_param_group({'params': g1, 'weight_decay': 0.0})  # add g1 (BatchNorm2d weights)
        LOGGER.info(f"{colorstr('optimizer:')} {type(optimizer).__name__}(lr={lr}) with parameter groups "
                    f"{len(g1)} weight(decay=0.0), {len(g0)} weight(decay={decay}), {len(g2)} bias")
        return optimizer